...
public abstract class Categorizer<T, K> implements Probability<T, K> {
    ...
    private Queue<CategoryCollection<T, K>> memoryQueue;
    ...
    public void reset() {
        this.featureCountPerCategory =
                new Hashtable<K, Dictionary<T,Integer>>(
                        Categorizer.INITIAL_CATEGORY_DICTIONARY_CAPACITY);
        this.totalFeatureCount =
                new Hashtable<T, Integer>(
                        Categorizer.INITIAL_FEATURE_DICTIONARY_CAPACITY);
        this.totalCategoryCount =
                new Hashtable<K, Integer>(
                        Categorizer.INITIAL_CATEGORY_DICTIONARY_CAPACITY);
        this.memoryQueue = new LinkedList<CategoryCollection<T, K>>();
    }
    ...
    public void incrementFeature(T feature, K category) {
        ...
        if (features == null) {
            this.featureCountPerCategory.put(category, new Hashtable<T, Integer>(Categorizer.INITIAL_FEATURE_DICTIONARY_CAPACITY));
    ...
    public float featureWeighedAverage(T feature, K category, Probability<T, K> calculator) {
        return this.featureWeighedAverage(feature, category, calculator, 1.0f, 0.5f);
    }

    public float featureWeighedAverage(T feature, K category, Probability<T, K> calculator, float weight) {
        return this.featureWeighedAverage(feature, category, calculator, weight, 0.5f);
    }

    public float featureWeighedAverage(T feature, K category, Probability<T, K> calculator, float weight, float assumedProbability) {
    ...
    public void learn(K category, Collection<T> features) {
        this.learn(new CategoryCollection<T, K>(features, category));
    }

    public void learn(CategoryCollection<T, K> classification) {
        ...
        if (this.memoryQueue.size() > this.memoryCapacity) {
            CategoryCollection<T, K> toForget = this.memoryQueue.remove();
    ...
    public abstract CategoryCollection<T, K> classify(Collection<T> features);
}